...
async function getOAuthToken() {
  try {
    const response = await axios.post('https://api.ntt.com/v1/oauth/accesstokens', {
...
// アクセストークン取得
app.get('/api/ocn/token', async (req, res) => {
  try {
...
// お問い合わせステータス取得
app.get("/api/contactUs/statuses", async (req, res) => {
  try {
...
app.post("/api/odata/post/noauth/group/tenant", async (req, res) => {
  try {
    const postData = req.body;
    const url = `${ODATA_BASE_URL}Tenants/Initialize`;
    const odataResponse = await axios.post(url, postData, {
      headers: {
...
app.post("/api/odata/post/group/tenant", async (req, res) => {
...
// OData 取得・認証なし
app.get("/api/odata/noAuth", async (req, res) => {
  try {
...
app.get('/api/odata/prefectures', async (req, res) => {
  const accessToken = req.headers.authorization;
...
// OData 新規レコード作成
app.post("/api/odata/post", async (req, res) => {
  const accessToken = req.headers.authorization;
...
// OData 部分的な更新
app.patch("/api/odata/patch", async (req, res) => {
  const accessToken = req.headers.authorization;
  ...
  try {
    ...
    const odataResponse = await axios.patch(url, patchData, {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': accessToken
      }
...
// OData 全体的な更新
app.put("/api/odata/put", async (req, res) => {
  const accessToken = req.headers.authorization;
  ...
  try {
    ...
    const odataResponse = await axios.put(url, putData, {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': accessToken
      }
...
// OData 削除
app.delete("/api/odata/delete", async (req, res) => {
  const accessToken = req.headers.authorization;
...
// OData ステージング取得
app.get("/api/odata/staging", async (req, res) => {
  try {
    // クライアントからのクエリパラメータ（odataQuery）を取得
    const { odataQuery } = req.query;
    ...
    const odataResponse = await axios.get(URI);
    const dataResult = odataResponse.data;
...
// OData ステージング削除
app.delete("/api/odata/staging/delete", async (req, res) => {
  try {
    const entitySet = req.query.entitySet;
    const entityId = req.query.id;
    ...
    if (axios.isAxiosError(err)) {
      ...
      console.error("Error status:", errorResponse?.status); // エラーステータスコード
      console.error("Error message:", errorResponse?.data);  // エラーメッセージ
      res.status(errorResponse?.status || 500).send("Server Error: " + errorResponse?.data);
    } else {
      // Axios以外のエラーの場合
...
// 回線一覧取得
app.get('/api/ocn/lines', async (req, res) => {
  try {
    const tokenData = await getOAuthToken();
    const page = req.query.page;
    const contractId = req.query.contractId;

    const response = await axios.get(`https://jp.api.ntt.com/v1/bocnmf/contracts/${contractId}/pages/1/lines`, {
      headers: {
...
// 回線のデータ利用量等の取得
app.get('/api/ocn/line/traffics', async (req, res) => {
  try {
    const tokenData = await getOAuthToken();
    const contractId = req.query.contractId;
    const lineNo = req.query.lineNo;

    const response = await axios.get(`https://jp.api.ntt.com/v1/bocnmf/contracts/${contractId}/lines/${lineNo}/traffic`, {
      headers: {
        'Authorization': `Bearer ${tokenData.accessToken}`,
      }
    });
...
app.get('/api/ocn/dataShare', async (req, res) => {
  try {
    const tokenData = await getOAuthToken();
    const contractId = req.query.contractId;

    const response = await axios.get(`https://jp.api.ntt.com/v1/bocnmf/contracts/${contractId}/dataShare`, {
      headers: {
        'Authorization': `Bearer ${tokenData.accessToken}`,
      }
    });
...
// 回線番号の容量シェア情報取得
app.get('/api/ocn/shared/line', async (req, res) => {
  try {
    const tokenData = await getOAuthToken();
    const contractId = req.query.contractId;
    const lineNo = req.query.line;

    const response = await axios.get(`https://jp.api.ntt.com/v1/bocnmf/contracts/${contractId}/lines/${lineNo}/dataShare/traffic`, {
      headers: {
        'Authorization': `Bearer ${tokenData.accessToken}`,
      }
    });
...
// 回線運用情報取得
app.get('/api/ocn/line', async (req, res) => {
  try {
    const tokenData = await getOAuthToken();
    const contractId = req.query.contractId;
    const lineNo = req.query.line;
...
// サスペンデッド有効化（使用中断）
app.put('/api/ocn/suspended', async (req, res) => {
  try {
    const { contractId, line } = req.body;
    const tokenData = await getOAuthToken();

    const response = await axios.put(`https://jp.api.ntt.com/v1/bocnmf/contracts/${contractId}/lines/${line}/suspend`, {}, {
      headers: {
        'Authorization': `Bearer ${tokenData.accessToken}`,
      }
    });
...
// サスペンデッド無効化（使用再開）
app.delete('/api/ocn/unSuspended', async (req, res) => {
  try {
    const { contractId, line } = req.body;
    const tokenData = await getOAuthToken();

    const response = await axios.delete(`https://jp.api.ntt.com/v1/bocnmf/contracts/${contractId}/lines/${line}/suspend`, {
      headers: {
        'Authorization': `Bearer ${tokenData.accessToken}`,
      }
    });
...
// 都道府県配下のテナント取得
app.get("/api/prefecture/tenants", async (req, res) => {
  try {
    ...
    // データ取得用クエリ
    const dataResult = await pool.query(dataQuery, params.filter(p => p !== undefined));
...
// リハビリステータス取得
app.get("/api/rehabilitation/statuses", async (req, res) => {
  try {
...
// お問い合わせ番号によるリハビリ取得
app.get("/api/contactNo", async (req, res) => {
  try {
...
// リハビリ一覧取得
app.get("/api/rehabilitations", async (req, res) => {
  try {
    const prefId = parseInt(req.query.prefId);
...
// テナントステータス取得
app.get("/api/facility/statuses/new", async (req, res) => {
  try {
...
// テナントステータス取得
app.get("/api/facility/statuses", async (req, res) => {
  try {
...
// テナント更新
app.put("/api/facility", async (req, res) => {
  try {
...
// 医療者取得（テナント配下）
app.get("/api/facility/staff", async (req, res) => {
  try {
    const tenantId = req.query.tenantId;
...
// 医療者取得（氏名検索）
app.get("/api/staff/name", async (req, res) => {
  try {
    const page = parseInt(req.query.page);
    const pageSize = parseInt(req.query.pageSize);
    const offset = (page - 1) * pageSize;
...
// 患者取得（テナント配下）
app.get("/api/facility/patient", async (req, res) => {
  try {
    const tenantId = req.query.tenantId;
    ...
    // データを取得
    const dataResult = await pool.query(dataQuery, dataParam);
...
app.get("/api/patients/name", async (req, res) => {
  try {
    ...
    // 総件数を取得
    const countResult = await pool.query(countQuery, countParam);
...
// デバイスステータス取得
app.get("/api/device/statuses", async (req, res) => {
  try {
...
app.get("/api/devices", async (req, res) => {
  try {
    ...
    const countResult = await pool.query(countQuery, countParams.filter(p => p !== undefined));
...
// デバイス一覧取得（シリアルナンバー）
app.get("/api/devices/serialNo", async (req, res) => {
  try {
    const page = parseInt(req.query.page);
    const pageSize = parseInt(req.query.pageSize);
    const offset = (page - 1) * pageSize;
    ...
    // データ取得用クエリ
    const dataResult = await pool.query(dataQuery, dataParams.filter(p => p !== undefined));
...
// デバイス一覧取得（患者ID）
app.get("/api/devices/patient/id", async (req, res) => {
  try {
    const page = parseInt(req.query.page);
    const pageSize = parseInt(req.query.pageSize);
    const offset = (page - 1) * pageSize;
...
// デバイス一覧取得（患者氏名）
app.get("/api/devices/patient/name", async (req, res) => {
  try {
    const page = parseInt(req.query.page);
    const pageSize = parseInt(req.query.pageSize);
    const offset = (page - 1) * pageSize;
...
// デバイス一覧取得（医療機関名）
app.get("/api/devices/institution/name", async (req, res) => {
  try {
    const page = parseInt(req.query.page);
    const pageSize = parseInt(req.query.pageSize);
    const offset = (page - 1) * pageSize;
...
app.get("/api/devices/patient/matching/name", async (req, res) => {
  try {
    const page = parseInt(req.query.page);
    const pageSize = parseInt(req.query.pageSize);
    const offset = (page - 1) * pageSize;
...
// タブレット取得（患者氏名）
app.get("/api/devices/tablet/patient/name", async (req, res) => {
  try {
    const page = parseInt(req.query.page);
    const pageSize = parseInt(req.query.pageSize);
    const offset = (page - 1) * pageSize;
...
// お知らせ種類取得
app.get("/api/notice/types", async (req, res) => {
  try {
...
// お問い合わせカテゴリー取得
app.get("/api/contactUs/categories", async (req, res) => {
  try {
...
// EMMトークン取得用の関数
async function getEMMToken(email, password) {
  try {
    const formData = new FormData();
    formData.append('email', email);
    formData.append('password', password);
    const response = await axios.post(`${EMM_BASE_URL}/auth/login`, formData);
...
// EMMアクセストークン取得
app.get('/api/emm/token', async (req, res) => {
  try {
    const accessToken = await getEMMToken('nmorikawa@remohab.com', '2q47112q');
...
// デバイスの再起動
app.post('/api/emm/reboot', async (req, res) => {
  try {
    const accessToken = await getEMMToken('nmorikawa@remohab.com', '2q47112q');
    const postData = {
...
// デバイス一覧の情報取得
app.get('/api/emm/devices', async (req, res) => {
  try {
    const accessToken = await getEMMToken('nmorikawa@remohab.com', '2q47112q');
...
// デバイス一覧の詳細情報取得
app.get('/api/emm/info/id', async (req, res) => {
  try {
    const accessToken = await getEMMToken('nmorikawa@remohab.com', '2q47112q');
...
// デバイスの情報取得
app.get('/api/emm/info', async (req, res) => {
  try {
    const accessToken = await getEMMToken('nmorikawa@remohab.com', '2q47112q');
...
// デバイスの休止状態
app.post('/api/emm/status', async (req, res) => {
  try {
    const accessToken = await getEMMToken('nmorikawa@remohab.com', '2q47112q');
    const postData = {
      imei: req.body.imei,
